...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 0110011", mulh , R, R(rd) = (SEXT(src1, 32) * SEXT(src2, 32)) >> 32);
  INSTPAT("0000000 ????? ????? 010 ????? 0110011", slt  , R, R(rd) = (sword_t)src1 < (sword_t)src2);
  ...
  INSTPAT("000000?????? ????? 001 ????? 0010011", slli , I, R(rd) = src1 << imm);
  ...
  INSTPAT("000000?????? ????? 101 ????? 0010011", srli , I, R(rd) = src1 >> imm);
  INSTPAT("010000?????? ????? 101 ????? 0010011", srai , I, R(rd) = (sword_t)src1 >> imm);
  INSTPAT("000000000000 00000 000 00000 1110011", ecall, I, s->dnpc = isa_raise_intr(R(17) /*a7*/, s->pc));
  INSTPAT("???????????? ????? 001 ????? 1110011", csrrw, I, word_t t = csr(imm); csr(imm) = src1; R(rd) = t);
  INSTPAT("???????????? ????? 010 ????? 1110011", csrrs, I, word_t t = csr(imm); csr(imm) = t | src1; R(rd) = t);
